...
class SafeTrainer(Trainer):
    ...
    def _maybe_log_save_evaluate(
        self, tr_loss, routing_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, start_time, learning_rate=None
    ):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            logs["routing_loss"] = round(routing_loss.item(), 4)
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step:
                        ...
                        # Freeze expert layers
                        # for name, param in model.named_parameters():
                        #     if '.experts.' in name or 'lm_head' in name:
                        #         param.requires_grad = False
                        #     else:
                        #         param.requires_grad = True
                        
                        # trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
                        # logger.warning(f"Safe training (Limiting trainable parameters: {trainable_params})")
                        ...
                        # Unfreeze expert layers
                        # for name, param in model.named_parameters():
                        #     param.requires_grad = True

                        # trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
                        # logger.warning(f"Safe training completed (Restoring trainable parameters: {trainable_params})")
            ...
            self._maybe_log_save_evaluate(
                tr_loss, routing_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, start_time, learning_rate=learning_rate
            )
