...
class SafeTrainer(Trainer):
    def __init__(self, train_dataset_safe, routing_logits_safe, temp, *args, **kwargs):
    ...
    def safe_training_step(self, model):
        safe_outputs = None
        safe_routing_logits = None

        batch_size = self.args.per_device_train_batch_size
        start = self.batch_id*batch_size
        end = min((self.batch_id+1)*batch_size, len(self.safe_dataset))
        ...
        routing_loss *= self.accelerator.num_processes

        self.batch_id += 1
        if self.batch_id * batch_size >= len(self.safe_dataset):
            self.batch_id = 0

        return routing_loss
        
    def _maybe_log_save_evaluate(
        self, tr_loss, ft_loss, routing_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, start_time, learning_rate=None
    ):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            ft_loss_scalar = self._nested_gather(ft_loss).mean().item()
            routing_loss_scalar = self._nested_gather(routing_loss).mean().item()
            ...
            logs["ft_loss"] = round(ft_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["routing_loss"] = round(routing_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def _inner_training_loop(
        ...
        ft_loss = torch.tensor(0.0, device=args.device)
        routing_loss = torch.tensor(0.0, device=args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        tr_loss_step, ft_loss_step, routing_loss_step = self.training_step(model, inputs, num_items_in_batch)
                    ...
                    if (
                        ...
                        ft_loss = ft_loss + ft_loss_step
                        routing_loss = routing_loss + routing_loss_step
            ...
            self._maybe_log_save_evaluate(
                tr_loss, ft_loss, routing_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, start_time, learning_rate=learning_rate
            )
    ...
    def training_step(
        ...
        with self.compute_loss_context_manager():
            ft_loss = self.compute_loss(model, inputs, num_items_in_batch=num_items_in_batch)

            routing_loss = self.safe_training_step(model)

            loss = ft_loss + routing_loss
        ...
        if self.use_apex:
            ...
            return loss.detach(), ft_loss.detach(), routing_loss.detach()