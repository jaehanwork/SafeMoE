...
with torch.no_grad():
    for batch in tqdm(dataloader):
        ...
        last_hidden_states.extend(outputs['logits'][:, -1].to(torch.float32).detach().cpu().tolist())

        directions_samples = [[] for _ in range(outputs['logits'].shape[0])]
        ...
        logits_samples = [[] for _ in range(outputs['logits'].shape[0])]
