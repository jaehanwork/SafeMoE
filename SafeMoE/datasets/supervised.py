...
from SafeMoE.datasets.utils import format_prompt, right_padding, get_system_prompt
...
class SupervisedDataset(TokenizedDataset):
    def preprocess(self, raw_sample: RawSample) -> SupervisedSample:
        ...
        if raw_sample.get('input') is not None:
            if raw_sample.get('system_prompt') is not None:
                system_prompt = raw_sample['system_prompt']
            else:
                system_prompt = get_system_prompt(self.tokenizer)
